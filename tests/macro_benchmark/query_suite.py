...
from common import APOLLO, CPU_TIME, MAX_MEMORY, WALL_TIME, get_absolute_path
...
class _QuerySuite:
    ...
    KNOWN_KEYS = {"config", "setup", "itersetup", "run", "iterteardown", "teardown", "common"}
    FORMAT = ["{:>24}", "{:>28}", "{:>16}", "{:>18}", "{:>22}", "{:>16}", "{:>16}", "{:>16}"]
    ...
    def __init__(self, args):
        ...
        argp.add_argument("--perf", default=False, action="store_true", help="Run perf on running tests and store data")
    ...
    def run(self, scenario, group_name, scenario_name, runner):
        ...
        for rerun_cnt in range(3):
            ...
            # warmup phase
            for _ in range(min(scenario_config.get("iterations", 1), scenario_config.get("warmup", 2))):
                execute("itersetup")
                execute("run")
            ...
            for iteration in range(num_iterations):
                ...
                if self.args.perf:
                    file_directory = "./perf_results/run_%d/%s/%s/" % (rerun_cnt, group_name, scenario_name)
                    ...
                    self.perf_proc = subprocess.Popen(
                        ["perf", "record", "-F", "999", "-g", "-o", path, "-p", database_pid]
                    )
    ...
    def append_scenario_summary(self, group_name, scenario_name, measurement_lists, num_iterations):
        ...
        for i, key in enumerate(
            ("parsing_time", "planning_time", "plan_execution_time", WALL_TIME, CPU_TIME, MAX_MEMORY)
        ):
...
class QueryParallelSuite(_QuerySuite):
    ...
    def runners(self):
        return {"MemgraphRunner": MemgraphParallelRunner, "NeoRunner": NeoParallelRunner}
...
class NeoRunner(_QueryRunner):
    ...
    def __init__(self, args):
        ...
        argp.add_argument(
            "--runner-config", default=get_absolute_path("config/neo4j.conf"), help="Path to neo config file"
        )
        self.args, remaining_args = argp.parse_known_args(args)
