...
def analyze_subgraph(
    ...
    g = nx.subgraph_view(MemgraphMultiDiGraph(ctx=context), lambda n: n in vertices, lambda n1, n2, e: e in edges)
    recs = _analyze_graph(context, g, analyses)
    return [mgp.Record(name=name, value=value) for name, value in recs]
...
def _articulation_points(g: nx.MultiDiGraph):
    ...
    return ("Number of articulation points", sum(1 for _ in nx.articulation_points(undirected)))
